...
// CUT begin
	static bool DoTest(string[] words, int __expected) {
		DateTime startTime = DateTime.Now;
		Exception exception = null;
		PrefixFreeSets instance = new PrefixFreeSets();
		int __result = 0;
		try {
			__result = instance.maxElements(words);
		}
	...
	public static void Main(string[] args){
		...
		Console.Error.WriteLine("PrefixFreeSets (500 Points)");
		Console.Error.WriteLine();

    	int nCases = 0, nPassed = 0;
    	using (var reader = File.OpenText("PrefixFreeSets.sample")) {
            while (true) {
                ...
                for (int i = 0; i < words.Length; ++i)
                    words[i] = (string) Convert.ChangeType(reader.ReadLine(), typeof(string));
                reader.ReadLine();
		...
		int T = (int)((DateTime.UtcNow - Jan1st1970).TotalSeconds - 1484548965);
		double PT = T / 60.0, TT = 75.0;
		Console.Error.WriteLine(string.Format("Time   : {0} minutes {1} secs", T / 60, T % 60));
		Console.Error.WriteLine(string.Format("Score  : {0:0.00} points", 500 * (0.3 + (0.7 * TT * TT) / (10.0 * PT * PT + TT * TT))));
	}
// CUT end
}
